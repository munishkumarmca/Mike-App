				...
				$itemHtml = '';	
					...
					if(!empty($mNavigation['sub_nav'])){
						...
						$active = ($currentController == $mNavigation['controller'] && $currentAction == $mNavigation['action'] ) ? 'active' : '';
							{
								...
								$itemHtml = '<li class = "parent '.$active.'" data-toggle="collapse" data-target="#menu-content-'.$uniqid.'">'.$this->Html->link($linkHtml, ["controller" => $mNavigation['controller'], "action" => $mNavigation['action']], ['class' => 'buttonq', 'escape' => false]);'</li>';	
								...
								foreach($mNavigation['sub_nav'] as $subNav){
									...
									{
										$childHtmlBody .= '<li class = "'.$activeI.'" ><i class="'.$subNav['fav_icon'].'"></i>'.$this->Html->link($subNav['link_text'], ["controller" => $subNav['controller'], "action" => $subNav['action']], ['class' => 'buttonq']);
									}
								}
								$childHtmlEnd = '</ul>';
						...
						{																					
							...
							$itemHtml = '<li class = "'.$active.'">'.$this->Html->link($linkHtml, ["controller" => $mNavigation['controller'], "action" => $mNavigation['action']], ['class' => 'buttonq', 'escape' => false]).'</li>';				
						}
					}*/
				$navHtml .= $itemHtml ;				
			}			
